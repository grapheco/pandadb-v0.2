  ...
  def onLeaf(plan: LogicalPlan): Pipe = {
    ...
    plan match {
      ...
      case AllNodesScan(ident, _) =>
        pipes.AllNodesScanPipe(ident)(id = id)
      ...
      case NodeByLabelScan(ident, label, _) =>
        pipes.NodeByLabelScanPipe(ident, LazyLabel(label))(id = id)
  ...
  def onOneChildPlan(plan: LogicalPlan, source: Pipe): Pipe = {
    ...
    plan match {
      ...
      case Selection(predicate, _) =>
        ...
        val pipe = pipes.FilterPipe(source, predicateExpression)(id = id)
        //NOTE: predicate push down
        if (nodeStore.isDefined) {
          source match {
            case x: pipes.AllNodesScanPipe =>
              x.pushDownPredicate(nodeStore.get, pipe, predicateExpression)
            case x: pipes.NodeByLabelScanPipe =>
      ...
      case Expand(_, fromName, dir, types: Seq[RelTypeName], toName, relName, ExpandAll) =>
        pipes.ExpandAllPipe(source, fromName, relName, toName, dir, LazyTypes(types.toArray))(id = id)
      ...
      case Create(_, nodes, relationships) =>
        pipes.CreatePipe(
          source,
          nodes.map(n =>
            pipes.CreateNodeCommand(n.idName, n.labels.map(LazyLabel.apply), n.properties.map(buildExpression))
          ).toArray,
          relationships.map(r =>
            pipes.CreateRelationshipCommand(r.idName, r.startNode, LazyType(r.relType.name), r.endNode, r.properties.map(buildExpression))
          ).toArray
        )(id = id)

      case MergeCreateNode(_, idName, labels, props) =>
        pipes.MergeCreateNodePipe(source,
          pipes.CreateNodeCommand(idName, labels.map(LazyLabel.apply), props.map(buildExpression))
        )(id = id)

      case MergeCreateRelationship(_, idName, startNode, typ, endNode, props) =>
        pipes.MergeCreateRelationshipPipe(source,
          pipes.CreateRelationshipCommand(idName, startNode, LazyType(typ)(semanticTable), endNode, props.map(buildExpression))
        )(id = id)
