...
public class Operations implements Write, ExplicitIndexWrite, SchemaWrite
{
    ...
    {
        ktx.assertOpen();
        nodeId = statement.reserveNode(nodeId);
        ktx.txState().nodeDoCreate( nodeId );
        return nodeId;
    }

    @Override
    public long nodeCreateWithLabels( long nodeId, int[] labels ) throws ConstraintValidationException
    {
        if ( labels == null || labels.length == 0 )
        {
            return nodeCreate(nodeId);
        }
            ...
            throws EntityNotFoundException
    {
        ...
        relationshipId = statement.reserveRelationship(relationshipId);
        ktx.txState().relationshipDoCreate( relationshipId, relationshipType, sourceNode, targetNode );
