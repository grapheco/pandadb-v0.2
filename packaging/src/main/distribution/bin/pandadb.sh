...
_read_config() {
  ...
  for file in "${PANDADB_CONF}"; do
    path="${PANDADB_CONF}/neo4j.conf"
    if [ -e "${path}" ]; then
      while read line; do
        parse_line "${line}"
      done <"${path}"
...
do_stop() {
	...
	if [ ! -f "$PANDADB_PIDFILE" ] || ! kill -0 "$(cat "$PANDADB_PIDFILE")"; then
		echo "Pandadb is not running..."
	else
		echo "stopping Pandadb..."
		PID="$(cat "$PANDADB_PIDFILE")"
		kill -9 $PID
		rm "$PANDADB_PIDFILE"
		echo "Pandadb stopped"
	fi
