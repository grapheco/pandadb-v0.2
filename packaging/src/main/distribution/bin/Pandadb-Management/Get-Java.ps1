...
<#
.SYNOPSIS
Retrieves information about Java on the local machine to start Pandadb programs

.DESCRIPTION
Retrieves information about Java on the local machine to start Pandadb services and utilities, tailored to the type of Pandadb edition

.PARAMETER PandadbServer
An object representing a valid Pandadb Server object

.PARAMETER ForServer
Retrieve the Java command line to start a Pandadb Server

.PARAMETER ForUtility
Retrieve the Java command line to start a Pandadb utility such as Pandadb Shell.

.PARAMETER StartingClass
The name of the starting class when invoking Java

.EXAMPLE
Get-Java -PandadbServer $serverObject -ForServer

Retrieves the Java command line to start the Pandadb server for the instance in $serverObject.
...
function Get-Java
{
  ...
  process
  {
    ...
    $EnvJavaHome = Get-PandadbEnv 'JAVA_HOME'
    $EnvClassPrefix = Get-PandadbEnv 'CLASSPATH_PREFIX'
    ...
    # Shell arguments for the PandadbServer and Arbiter classes
    $ShellArgs = @()
    if ($PsCmdlet.ParameterSetName -eq 'ServerInvoke')
    {
      ...
      $ClassPath = "$($PandadbServer.Home)/lib/*;$($PandadbServer.Home)/plugins/*"
      $ShellArgs = @("-cp `"$($ClassPath)`"" `
          ,'-server' `
          ,'-Dlog4j.configuration=file:conf/log4j.properties' `
          ,'-Dpandadb.ext.udc.source=zip-powershell' `
          ,'-Dorg.neo4j.cluster.logdirectory=data/log' `
        )

      # Parse Java config settings - Heap initial size
      $option = (Get-PandadbSetting -Name 'dbms.memory.heap.initial_size' -PandadbServer $PandadbServer)
      if ($option -ne $null) {
        $mem = "$($option.Value)"
...
"@
      ...
      $option = (Get-PandadbSetting -Name 'dbms.memory.heap.max_size' -PandadbServer $PandadbServer)
      if ($option -ne $null) {
        $mem = "$($option.Value)"
        if ($mem -notmatch '[\d]+[gGmMkK]') {
          $mem += "m"
...
"@
      ...
      $option = (Get-PandadbSetting -Name 'dbms.jvm.additional' -PandadbServer $PandadbServer)
      if ($option -ne $null) { $ShellArgs += $option.value }

      # Parse Java config settings - GC
      $option = (Get-PandadbSetting -Name 'dbms.logs.gc.enabled' -PandadbServer $PandadbServer)
      if (($option -ne $null) -and ($option.value.ToLower() -eq 'true')) {
        if ($javaVersion.isJava8) {
          # JAVA 8 GC logs configuration
          $ShellArgs += "-Xloggc:`"$($PandadbServer.LogDir)/gc.log`""

          $option = (Get-PandadbSetting -Name 'dbms.logs.gc.options' -PandadbServer $PandadbServer)
          if ($option -eq $null) {
          ...
          $option = (Get-PandadbSetting -Name 'dbms.logs.gc.rotation.size' -PandadbServer $PandadbServer)
          if ($option -ne $null) {
          ...
          $option = (Get-PandadbSetting -Name 'dbms.logs.gc.rotation.keep_number' -PandadbServer $PandadbServer)
          if ($option -ne $null) {
          ...
          $gcFile = "\`"" + "$($PandadbServer.LogDir)/gc.log" + "\`""
      ...
      $ShellArgs += @("-Dfile.encoding=UTF-8",
        $serverMainClass,
        "--config-dir=`"$($PandadbServer.ConfDir)`"",
        "--home-dir=`"$($PandadbServer.Home)`"")
    }
    ...
    if ($PsCmdlet.ParameterSetName -eq 'UtilityInvoke')
    {
      ...
      $ClassPath = "$($PandadbServer.Home)/lib/*;$($PandadbServer.Home)/bin/*"
      ...
      $ShellArgs += @("-XX:+UseParallelGC",
        ...
        "-Dbasedir=`"$($PandadbServer.Home)`"",`
           '-Dfile.encoding=UTF-8')

      # Determine user configured heap size.
      $HeapSize = Get-PandadbEnv 'HEAP_SIZE'
      if ($HeapSize -ne $null) {
