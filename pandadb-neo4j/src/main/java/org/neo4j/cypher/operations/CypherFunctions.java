...
import org.neo4j.values.virtual.NodeReferencePromised;
...
public final class CypherFunctions
{
    ...
    {
        if ( item instanceof NodeValue || item instanceof NodeReferencePromised)
        {
            return access.getLabelsForNode( ((VirtualNodeValue) item).id() );
        }
        else
        {
    ...
    public static boolean hasLabel( AnyValue entity, int labelToken, DbAccess access )
    {
        ...
        {
            return access.isLabelSetOnNode( labelToken, ((VirtualNodeValue) entity).id() );
        }
        else
        {
