...
public class RecordStorageReader implements StorageReader
{
     ...
     * if the given id value exceeds the limit of current highId. The function-setHighestPossibleIdInUse() is synchronized,
     * so we may use a little bigger value as the highId to prevent frequent calls it.
     * The implementation here is unsafe.
     * @param id
     * @return
     */
    @Override
    public long reserveNode(long id)
    {
        if (id >= nodeStore.getHighestPossibleIdInUse()) {
            nodeStore.setHighestPossibleIdInUse(id + 20);
        }
        return id;
    }

    @Override
    public long reserveRelationship(long id)
    {
        if (id >= relationshipStore.getHighestPossibleIdInUse()) {
            relationshipStore.setHighestPossibleIdInUse(id + 20);
        }
        return id;
    }
    // END-NOTE: pandadb
