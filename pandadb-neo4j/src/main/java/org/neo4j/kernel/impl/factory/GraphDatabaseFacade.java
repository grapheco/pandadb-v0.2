...
public class GraphDatabaseFacade implements GraphDatabaseAPI, EmbeddedProxySPI
{
    ...
    public Node createNode(long nodeId)
    {
        ...
        try ( Statement ignore = transaction.acquireStatement() )
        {
            return newNodeProxy( transaction.dataWrite().nodeCreate(nodeId) );
        }
        catch ( InvalidTransactionTypeKernelException e )
        {
            throw new ConstraintViolationException( e.getMessage(), e );
    ...
    @Override
    public Node createNode(long nodeId, Label... labels )
    {
        KernelTransaction transaction = statementContext.getKernelTransactionBoundToThisThread( true );
        try ( Statement ignore = transaction.acquireStatement() )
        {
            ...
            nodeId = write.nodeCreateWithLabels(nodeId, labelIds );
